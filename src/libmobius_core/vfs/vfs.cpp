...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
vfs::impl::impl (const mobius::core::pod::map& state)
{
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  // Create disks
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  if (state.contains ("disks"))
    {
      auto disks = state.get <std::vector <mobius::core::pod::data>> ("disks");
  ...
  //! \deprecated since=2.5 old datasources
    ...
    {
      auto disks = state.get <std::vector <mobius::core::pod::data>> ("datasources");
  ...
  if (state.contains ("blocks"))
    {
      auto blocks = state.get <std::vector <mobius::core::pod::data>> ("blocks");

      // Build blocks
      for (const auto& b_data : blocks)
        {
          auto b_state = mobius::core::pod::map (b_data);
          blocks_.push_back (mobius::core::vfs::block (b_state));
        }
...
vfs::impl::get_state () const
{
  ...
  // Datasources
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  std::vector <mobius::core::pod::data> disks;
  ...
  // Blocks
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  std::vector <mobius::core::pod::data> blocks;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
std::size_t
vfs::impl::add_disk (const mobius::core::vfs::disk& d)
{
  disks_.push_back (d);
  rescan ();
...
// @param state Object state
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
vfs::vfs (const mobius::core::pod::map& state)
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
std::size_t
vfs::add_disk (const mobius::core::vfs::disk& d)
{
