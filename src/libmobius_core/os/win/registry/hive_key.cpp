...
hive_key::impl::_load_metadata () const
{
    ...
    if (classname_offset != INVALID_OFFSET)
    {
        ...
        if (cellsize <= 0)
            classname_ =
                decoder.get_string_by_size (classname_length, "UTF-16LE");
    }
...
hive_key::impl::_load_subkeys () const
{
    ...
    // check if it can retrieve data
    if (offset_ == INVALID_OFFSET || subkeys_offset_ == INVALID_OFFSET ||
        subkeys_offset_ >= reader_.get_size ())
        return;
...
hive_key::impl::_load_values () const
{
    ...
    // check if it can retrieve data
    if (offset_ == INVALID_OFFSET || values_offset_ == INVALID_OFFSET ||
        values_offset_ >= reader_.get_size ())
        return;
