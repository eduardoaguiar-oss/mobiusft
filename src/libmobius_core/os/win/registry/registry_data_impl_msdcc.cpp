...
decrypt_cts_aes_128 (const mobius::core::bytearray &key,
                     ...
                     const mobius::core::bytearray &ciphertext)
{
    ...
    if (ciphertext.size () >= block_size * 2)
    {
        ...
        if (padsize < block_size)
        {
            ...
            mobius::core::bytearray::size_type pos =
                ciphertext.size () - block_size * 2 + padsize;
            mobius::core::bytearray dn =
                aes1.decrypt (ciphertext.slice (pos, pos + block_size - 1));
        ...
        mobius::core::bytearray::size_type pos =
            l_ciphertext.size () - block_size * 2;
        l_ciphertext =
            l_ciphertext.slice (0, pos - 1) +
            l_ciphertext.slice (pos + block_size, pos + block_size * 2 - 1) +
            l_ciphertext.slice (pos, pos + block_size - 1);
    }
...
// @see
// https://github.com/Neohapsis/creddump7/blob/master/framework/win32/domcachedump.py
...
registry_data_impl_msdcc::_load_data () const
{
    ...
    mobius::core::bytearray data =
        decoder.get_bytearray_by_size (encrypted_data_.size () - 96);
    ...
    if (encrypted)
    {
        ...
        else if (algorithm == 10)
        {
            ...
            data_ = encrypted_data_.slice (0, 95) +
                    decrypt_cts_aes_128 (key, iv, data);
        }
