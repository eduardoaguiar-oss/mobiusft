...
reader_impl_device::reader_impl_device (const device& dev)
{
  ...
  const std::string sector_size_str = dev.get_sysattr ("queue/logical_block_size");

  if (sectors_str.empty () || sector_size_str.empty ())
    throw std::invalid_argument (MOBIUS_EXCEPTION_MSG ("unable to get device size"));
...
{
  std::string value;
  const char *p_value = udev_device_get_property_value (impl_.get (), name.c_str ());
...
device::get_property_list () const
{
  ...
  while (p)
    {
      property_list[udev_list_entry_get_name (p)] = udev_list_entry_get_value (p);
...
{
  std::string value;
  const char *p_value = udev_device_get_sysattr_value (impl_.get (), name.c_str ());
