...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
std::string
conv_charset_to_utf8 (const mobius::core::bytearray& data, const std::string& charset)
{
...
conv_charset_to_utf8_partial (const mobius::core::bytearray& data, const std::string& charset)
{
  const std::string u_charset = mobius::core::string::toupper (charset);
  ...
  if (::iconv (cd, &inbuf, &insize, &p_out, &outsize) != (size_t) -1)
    {
      ...
      out = mobius::core::bytearray (reinterpret_cast <const uint8_t *> (outbuf.get ()), p_out - outbuf.get ());
    }
  ...
  if (rc != (size_t) -1 || errno == EINVAL)
    {
      ...
      if (inbuf < (char *) data.end ())
        rest = mobius::core::bytearray (reinterpret_cast <const uint8_t *> (inbuf), (char *) data.end () - inbuf);
    }
