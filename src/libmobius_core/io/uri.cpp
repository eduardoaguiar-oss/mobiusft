...
static constexpr const char *UNRESERVED_CHARS =
...
static constexpr const char *ALLOWED_AUTHORITY =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~!$&'()*+"
    ",;=:@[]";

// @brief RFC 3986 - section 3.3
static constexpr const char *ALLOWED_PATH =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~!$&'()*+"
    ",;=:@/";

// @brief RFC 3986 - section 3.4
static constexpr const char *ALLOWED_QUERY =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~!$&'()*+"
    ",;=:@/?";
...
static constexpr const char *URI_PATTERN =
    "(([A-Z][A-Z0-9+.-]*):)?(//([^/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?";
static std::regex URI_REGEX (URI_PATTERN,
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static std::string
merge_paths (const mobius::core::io::uri &base,
             const mobius::core::io::uri &rel)
{
    std::string path;
...
uri::uri (const std::string &value)
{
    ...
    // Set attributes
    scheme_ =
        mobius::core::string::tolower (_decode_triplets (match[2].str ()));
    query_ = match[7].str ();
    ...
    // If it is not a relative URI, remove dot segments - RFC 3986 -
    // section 6.2.2.3
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
uri::uri (const std::string &scheme, const std::string &username,
...
uri::_set_authority (const std::string &value)
{
    ...
    host_ = mobius::core::string::tolower (_decode_triplets (host));
...
uri::get_path (const std::string &encoding) const
{
    ...
    return mobius::core::conv_charset_to_utf8 (_decode_triplets (path_),
                                               encoding);
}
...
uri::get_query (const std::string &encoding) const
{
    ...
    return mobius::core::conv_charset_to_utf8 (_decode_triplets (query_),
                                               encoding);
}
...
uri::get_fragment (const std::string &encoding) const
{
    ...
    return mobius::core::conv_charset_to_utf8 (_decode_triplets (fragment_),
                                               encoding);
}
...
uri::get_sibling_by_name (const std::string &filename) const
{
    ...
    return uri (scheme_, username_, password_, host_, port_,
                cpath.get_value ());
}
...
uri::get_filename (const std::string &encoding) const
{
    ...
    return mobius::core::conv_charset_to_utf8 (_decode_triplets (value),
                                               encoding);
}
...
join (const uri &base, const uri &rel)
{
    ...
    if (base.get_scheme ().empty ())
        throw std::invalid_argument (
            MOBIUS_EXCEPTION_MSG ("base URI has no scheme"));
    ...
    else
    {
        ...
        else
        {
            ...
            {
                path = base.get_path ();
                query = rel.get_query ().empty () ? base.get_query ()
                                                  : rel.get_query ();
            }
            else
            {
