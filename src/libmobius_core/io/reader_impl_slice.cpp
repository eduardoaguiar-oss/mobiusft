...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
reader_impl_slice::reader_impl_slice (mobius::core::io::reader reader,
                                      offset_type pos, offset_type end)
    : reader_ (reader)
{
    if (pos < 0)
        pos += reader.get_size ();
    ...
    if (pos < 0)
        throw std::invalid_argument (
            MOBIUS_EXCEPTION_MSG ("invalid start position"));

    if (end < pos)
        throw std::invalid_argument (
            MOBIUS_EXCEPTION_MSG ("invalid end position"));
