  ...
  if (!user.empty () || !passwd.empty () || !workgroup.empty ())
    {
      ...
      if (!context)
        throw std::runtime_error (MOBIUS_EXCEPTION_MSG ("could not allocate smbc context"));
	  ...
	  smbc_free_context(context, 0);
          throw std::runtime_error (MOBIUS_EXCEPTION_MSG ("could not initialize smbc context"));
        }
