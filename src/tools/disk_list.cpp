      ...
      if (dev.get_type () == "disk" && dev.get_property ("ID_CDROM") != "1")
        {
          ...
          std::cout << "serial: " << dev.get_property ("ID_SERIAL_SHORT") << std::endl;
          std::cout << "sectors: " << dev.get_sysattr ("size") << std::endl;
          std::cout << "sector size: " << dev.get_sysattr ("queue/logical_block_size") << " bytes" << std::endl;

          auto reader = dev.new_reader ();
          std::cout << "total size: " << reader.get_size () << " bytes" << std::endl;
          std::cout << std::endl;
          ...
          for (int i = 0; i < 32; i++)
            {
              ...
              for (auto c : data)
                std::cout << " " << std::hex << std::setfill ('0') << std::setw (2) << int (c) << std::dec;
