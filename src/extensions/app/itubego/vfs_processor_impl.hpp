#ifndef MOBIUS_EXTENSION_APP_ITUBEGO_VFS_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_ITUBEGO_VFS_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief iTubeGo <i>vfs_processor</i> implementation class
