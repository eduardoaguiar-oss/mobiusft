#ifndef MOBIUS_EXTENSION_APP_ARES_FILE_SHAREL_HPP
#define MOBIUS_EXTENSION_APP_ARES_FILE_SHAREL_HPP
...
public:
  ...
  // Prototypes
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  file_sharel (const mobius::core::io::reader&);
