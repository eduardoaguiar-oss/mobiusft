...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
_populate_metadata (mobius::extension::app::ares::file_shareh::entry &e,
                    const mobius::core::bytearray &data)
{
         ...
         mobius::extension::app::ares::decode_metadata (data))
    {
        ...
        case 2:
            ...
            break;
        case 3:
            e.artist = v.to_string ();
            break;
        case 4:
            e.album = v.to_string ();
            break;
        case 5:
            e.category = v.to_string ();
            break;
        case 6:
            e.year = v.to_string ();
            break;
        case 8:
            e.language = v.to_string ();
            break;
        case 9:
            e.url = v.to_string ();
            break;
        case 10:
            e.comment = v.to_string ();
            break;
        case 11:
        {
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
file_shareh::file_shareh (const mobius::core::io::reader &reader)
{
    ...
    while (decoder)
    {
        ...
        std::uint16_t data_size =
            data[21] | (static_cast<std::uint16_t> (data[22]) << 8);
