...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
decoder_impl_pbthash::decode (const mobius::core::io::reader &reader)
{
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    // Decode file
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    metadata_.set_value ("general", "bytes_downloaded",
                         pbthash.get_bytes_downloaded ());
    metadata_.set_value ("general", "bytes_uploaded",
                         pbthash.get_bytes_uploaded ());
    metadata_.set_value ("general", "pieces_count",
                         pbthash.get_pieces_count ());
    metadata_.set_value ("general", "files_count", pbthash.get_files_count ());
    metadata_.set_value ("general", "torrent_path",
                         pbthash.get_torrent_path ());
    metadata_.set_value ("general", "torrent_name",
                         pbthash.get_torrent_name ());
    metadata_.set_value ("general", "name", pbthash.get_name ());
    metadata_.set_value ("general", "comment", pbthash.get_comment ());
    metadata_.set_value ("general", "download_started_time",
                         pbthash.get_download_started_time ());
    metadata_.set_value ("general", "elapsed_time",
                         pbthash.get_elapsed_time ());
    ...
    for (const auto &p : pbthash.get_pieces ())
        pieces.emplace_back (
            mobius::core::pod::data {p.hash_sha1, p.is_checked});
    ...
    for (const auto &f : pbthash.get_files ())
        files.emplace_back (mobius::core::pod::data {f.name, f.path, f.size,
