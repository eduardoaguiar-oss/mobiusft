#ifndef MOBIUS_EXTENSION_APP_ARES_DECODER_IMPL_ARESTRA_H
#define MOBIUS_EXTENSION_APP_ARES_DECODER_IMPL_ARESTRA_H
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
class decoder_impl_arestra : public mobius::core::file_decoder::decoder_impl_base
{
public:
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
