#ifndef MOBIUS_EXTENSION_APP_ARES_DECODER_IMPL_TORRENTH_HPP
#define MOBIUS_EXTENSION_APP_ARES_DECODER_IMPL_TORRENTH_HPP
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
class decoder_impl_torrenth : public mobius::core::file_decoder::decoder_impl_base
{
public:
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
