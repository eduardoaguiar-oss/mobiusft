#ifndef MOBIUS_EXTENSION_APP_ARES_FILE_ARESTRA_H
#define MOBIUS_EXTENSION_APP_ARES_FILE_ARESTRA_H
...
public:
  ...
  // Prototypes
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  file_arestra (const mobius::io::reader&);
...
private:
  ...
  // Helper functions
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  void _populate_metadata (const mobius::bytearray&);
};
