#ifndef MOBIUS_EXTENSION_APP_STICKY_NOTES_VFS_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_STICKY_NOTES_VFS_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Sticky Notes <i>vfs_processor</i> implementation class
