...
static std::unordered_set<std::int64_t> UNKNOWN_SCHEMA_VERSIONS = {
    ...
    555,  556,  557,  558,  559,  560,  562,  563,  564,  565,  566,  567,
    ...
    676,  677,  678,  679,  681,  682,  683,  684,  685,  686,  687,  688,
    ...
    833,  834,  836,  837,  838,  839,  840,  841,  842,  843,  844,  845,
...
file_skype_db::_load_corelib_messages (mobius::core::database::database &db)
{
    ...
    try
    {
        ...
        while (stmt.fetch_row ())
        {
            ...
            obj.body_xml = stmt.get_column_string (4);
            obj.bots_settings = stmt.get_column_string (5);
            obj.call_guid = stmt.get_column_string (6);
            obj.charmsg_type = stmt.get_column_int64 (7);
            obj.chatmsg_status = stmt.get_column_int64 (8);
            ...
            obj.extprop_mms_msg_metadata = stmt.get_column_string (18);
            obj.extprop_sms_server_id = stmt.get_column_string (19);
            obj.extprop_sms_src_msg_id = stmt.get_column_string (20);
            obj.extprop_sms_sync_global_id = stmt.get_column_string (21);
            obj.from_dispname = stmt.get_column_string (22);
            obj.guid = stmt.get_column_bytearray (23);
            obj.id = stmt.get_column_int64 (24);
            obj.identities = stmt.get_column_string (25);
            obj.is_parmanent = stmt.get_column_int64 (26);
            obj.language = stmt.get_column_string (27);
            obj.leavereason = stmt.get_column_int64 (28);
            obj.newoptions = stmt.get_column_int64 (29);
            obj.newrole = stmt.get_column_int64 (30);
            obj.oldoptions = stmt.get_column_int64 (31);
            obj.option_bits = stmt.get_column_int64 (32);
            obj.param_key = stmt.get_column_int64 (33);
            obj.param_value = stmt.get_column_int64 (34);
            obj.participant_count = stmt.get_column_int64 (35);
            obj.pk_id = stmt.get_column_int64 (36);
            obj.reaction_thread = stmt.get_column_string (37);
            obj.reason = stmt.get_column_string (38);
            obj.remote_id = stmt.get_column_int64 (39);
            obj.sending_status = stmt.get_column_int64 (40);
            obj.server_id = stmt.get_column_int64 (41);
            obj.timestamp = get_datetime (stmt.get_column_int64 (42));
            obj.timestamp_ms = stmt.get_column_int64 (43);
            obj.type = stmt.get_column_int64 (44);
            ...
            try
            {
                ...
                obj.parsed_content = parser.get_richtext ();

                if (!obj.parsed_content && !obj.body_xml.empty ())
                    obj.parsed_content.add_text (obj.body_xml);
            }
            catch (const std::exception &e)
            {
                log.warning (__LINE__, e.what ());
                log.warning (__LINE__, "Raw message content: " + obj.body_xml);
            }

            // Set conversation data
            auto iter = conv_identity.find (obj.convo_id);

            if (iter != conv_identity.end ())
            {
                obj.conversation_identity = iter->second.first;
                obj.conversation_display_name = iter->second.second;
            }

            // Add messages to the list
            corelib_messages_.emplace_back (std::move (obj));
        }
    }
    catch (const std::exception &e)
    {
...
file_skype_db::_load_messages (mobius::core::database::database &db)
{
    ...
    try
    {
        ...
        while (stmt.fetch_row ())
        {
            ...
            if (!obj.conversation_type.empty () &&
                !obj.conversation_id.empty ())
            ...
            try
            {
                ...
                if (!obj.parsed_content)
                    obj.parsed_content.add_text (obj.content);
            }
            catch (const std::exception &e)
            {
                log.warning (__LINE__, e.what ());
