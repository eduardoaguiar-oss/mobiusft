#ifndef MOBIUS_EXTENSION_APP_SKYPE_EVIDENCE_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_SKYPE_EVIDENCE_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Skype <i>evidence_processor</i> implementation class
