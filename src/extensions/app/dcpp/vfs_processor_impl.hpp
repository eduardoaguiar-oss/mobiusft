#ifndef MOBIUS_EXTENSION_APP_DCPP_VFS_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_DCPP_VFS_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief DC++ <i>vfs_processor</i> implementation class
