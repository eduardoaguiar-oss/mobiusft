#ifndef MOBIUS_EXTENSION_APP_UTORRENT_EVIDENCE_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_UTORRENT_EVIDENCE_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief uTorrent <i>evidence_processor</i> implementation class
