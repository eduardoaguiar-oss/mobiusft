...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Handle evidence loaded event
...
)
{
    ...
    try
    {
        ...
        if (type == "cookie")
            ...
            _process_encryption_key (e);

        else if (type == "password")
            _process_password (e);

        else
            _process_generic_evidence (e);
    }
    catch (const std::exception &e)
    {
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_processor_impl::_process_cookie (mobius::framework::model::evidence e)
{
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    if (rc)
    {
        // prune first 32 bytes if encryption algorithm is "v20"
        if (encrypted_value.startswith ("v20"))
            decrypted_value = left_prune (decrypted_value, 32);

        e.set_attribute ("value", decrypted_value);
        e.remove_attribute ("encrypted_value");

        // notify coordinator
        mediator_.on_evidence_attribute_modified (e, "value");
    }

    else
    {
        // Store the evidence for later processing
        pending_evidences_.push_back (e);
    }
}
...
)
{
    ...
    if (value)
    {
        _process_new_encryption_key (key_type, id, value);
        return;
    }
    ...
    if (key_type == "chromium.v10")
    {
        if (encrypted_value)
        {
            ...
            if (rc)
            {
                ...
                _process_new_encryption_key (key_type, id, decrypted_value);
            }
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Process generic evidence
// @param evidence Evidence to process
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
    ...
    for (const auto &name : iter->second)
    {
        ...
        if (encrypted_value)
        {
            ...
            if (rc)
            {
                ...
                mediator_.on_evidence_attribute_modified (evidence, name);
            }
            else
            {
                is_encrypted = true;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Process password evidence
// @param evidence Evidence to process
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
...
)
{
    ...
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    // Attempt to decrypt password value
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    auto [rc, decrypted_value] = _decrypt_data (encrypted_value);

    if (rc)
    {
        evidence.set_attribute ("value", decrypted_value.to_string ());
        evidence.remove_attribute ("encrypted_value");
        mediator_.on_evidence_attribute_modified (evidence, "value");
    }
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Decrypt v10/v20 value
// @param data Encrypted v10/v20 data to decrypt
// @return Decrypted data as bytearray
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
std::pair<bool, mobius::core::bytearray>
...
) const
{
    ...
    {
        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        // Try to decrypt encrypted value with DPAPI system key first
        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        // Try to decrypt encrypted value with DPAPI user key second
        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
// @note This function processes pending evidences that can be decrypted
// with the newly stored key. It checks the type of each pending evidence
