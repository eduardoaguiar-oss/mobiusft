#ifndef MOBIUS_EXTENSION_APP_EMULETORRENT_EVIDENCE_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_EMULETORRENT_EVIDENCE_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief EmuleTorrent <i>evidence_processor</i> implementation class
