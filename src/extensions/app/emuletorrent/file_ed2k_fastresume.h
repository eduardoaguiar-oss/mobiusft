#ifndef MOBIUS_EXTENSION_APP_EMULETORRENT_FILE_ED2K_FASTRESUME_H
#define MOBIUS_EXTENSION_APP_EMULETORRENT_FILE_ED2K_FASTRESUME_H
...
public:
  ...
  // Prototypes
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  file_ed2k_fastresume (const mobius::io::reader&);
