...
file_bt_fastresume::_load_metadata (const mobius::core::pod::map &metadata)
{
    ...
    metadata_.set (
        "libtorrent_version", metadata.get<std::string> ("libtorrent-version")
    );
    metadata_.set (
        "num_downloaders", metadata.get<std::int64_t> ("num_downloaders")
    );
    metadata_.set ("num_complete", metadata.get<std::int64_t> ("num_complete"));
    metadata_.set (
        "num_incomplete", metadata.get<std::int64_t> ("num_incomplete")
    );
         ...
         metadata.get<std::vector<mobius::core::pod::data>> ("trackers"))
    {
        ...
        std::copy (
            trackers.begin (), trackers.end (), std::back_inserter (trackers_)
        );
    }
    ...
    decoder = mobius::core::decoder::data_decoder (
        metadata.get<mobius::core::bytearray> ("peers6")
    );
