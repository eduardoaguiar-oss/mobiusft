...
show_cqueryhit (const mobius::extension::app::shareaza::CQueryHit& qh, int indent)
{
  ...
  std::cerr << indent_text << "Protocol ID: " << qh.get_protocol_id () << std::endl;
  std::cerr << indent_text << "Protocol name: " << qh.get_protocol_name () << std::endl;
  std::cerr << indent_text << "Search ID: " << qh.get_search_id () << std::endl;
  std::cerr << indent_text << "Client ID: " << qh.get_client_id () << std::endl;
  ...
  std::cerr << indent_text << "Str. speed: " << qh.get_str_speed () << std::endl;
  std::cerr << indent_text << "Vendor code: " << qh.get_vendor_code () << std::endl;
  std::cerr << indent_text << "Vendor name: " << qh.get_vendor_name () << std::endl;
  std::cerr << indent_text << "Tri busy: " << qh.get_tri_busy () << std::endl;
  std::cerr << indent_text << "Tri push: " << qh.get_tri_push () << std::endl;
  std::cerr << indent_text << "Tri stable: " << qh.get_tri_stable () << std::endl;
  std::cerr << indent_text << "Tri measured: " << qh.get_tri_measured () << std::endl;
  ...
  std::cerr << indent_text << "Flag browse host: " << qh.get_b_browse_host () << std::endl;
  std::cerr << indent_text << "Flag matched: " << qh.get_b_matched () << std::endl;
  std::cerr << indent_text << "Flag size: " << qh.get_b_size () << std::endl;
  std::cerr << indent_text << "Flag preview: " << qh.get_b_preview () << std::endl;
  std::cerr << indent_text << "Flag collection: " << qh.get_b_collection () << std::endl;
  std::cerr << indent_text << "Flag bogus: " << qh.get_b_bogus () << std::endl;
  std::cerr << indent_text << "Flag download: " << qh.get_b_download () << std::endl;
  std::cerr << indent_text << "Flag exact match: " << qh.get_b_exact_match () << std::endl;
  std::cerr << indent_text << "Hash SHA-1: " << qh.get_hash_sha1 () << std::endl;
  std::cerr << indent_text << "Hash Tiger: " << qh.get_hash_tiger () << std::endl;
  std::cerr << indent_text << "Hash ED2K: " << qh.get_hash_ed2k () << std::endl;
  ...
  std::cerr << indent_text << "Hit sources: " << qh.get_hit_sources () << std::endl;
  std::cerr << indent_text << "Partial: " << qh.get_partial () << std::endl;
  std::cerr << indent_text << "Preview: " << qh.get_preview () << std::endl;
  std::cerr << indent_text << "Schema URI: " << qh.get_schema_uri () << std::endl;
  std::cerr << indent_text << "Schema Plural: " << qh.get_schema_plural () << std::endl;
...
show_cquerysearch (const mobius::extension::app::shareaza::CQuerySearch& qs, int indent)
{
  ...
  std::cerr << indent_text << "Hash SHA-1: " << qs.get_hash_sha1 () << std::endl;
  std::cerr << indent_text << "Hash Tiger: " << qs.get_hash_tiger () << std::endl;
  std::cerr << indent_text << "Hash ED2K: " << qs.get_hash_ed2k () << std::endl;
  ...
  std::cerr << indent_text << "Flag want URL: " << qs.get_flag_want_url () << std::endl;
  std::cerr << indent_text << "Flag want DN: " << qs.get_flag_want_dn () << std::endl;
  std::cerr << indent_text << "Flag want XML: " << qs.get_flag_want_xml () << std::endl;
  std::cerr << indent_text << "Flag want COM: " << qs.get_flag_want_com () << std::endl;
  std::cerr << indent_text << "Flag want PFS: " << qs.get_flag_want_pfs () << std::endl;
  std::cerr << indent_text << "Min. size: " << qs.get_min_size () << std::endl;
  std::cerr << indent_text << "Max. size: " << qs.get_max_size () << std::endl;
}
...
show_cmanagedsearch (const mobius::extension::app::shareaza::CManagedSearch& ms, int indent)
{
  ...
  std::cerr << indent_text << "Flag active: " << ms.get_flag_active () << std::endl;
  std::cerr << indent_text << "Flag receive: " << ms.get_flag_receive () << std::endl;
  std::cerr << indent_text << "Allow G1: " << ms.get_allow_g1 () << std::endl;
  std::cerr << indent_text << "Allow G2: " << ms.get_allow_g2 () << std::endl;
  std::cerr << indent_text << "Allow ED2K: " << ms.get_allow_ed2k () << std::endl;
...
show_cmatchfile (const mobius::extension::app::shareaza::CMatchFile& mf)
{
  ...
  std::cerr << "\t\t\tFlag expanded: " << mf.get_flag_expanded () << std::endl;
  std::cerr << "\t\t\tFlag existing: " << mf.get_flag_existing () << std::endl;
  std::cerr << "\t\t\tFlag download: " << mf.get_flag_download () << std::endl;
  std::cerr << "\t\t\tFlag one valid: " << mf.get_flag_one_valid () << std::endl;
...
show_cmatchlist (const mobius::extension::app::shareaza::CMatchList& ml)
{
  ...
  std::cerr << "\t\tFilter unstable: " << ml.get_filter_unstable () << std::endl;
  ...
  std::cerr << "\t\tFilter suspicious: " << ml.get_filter_suspicious () << std::endl;
  std::cerr << "\t\tRegexp: " << ml.get_regexp () << std::endl;
  std::cerr << "\t\tFilter min. size: " << ml.get_filter_min_size () << std::endl;
  std::cerr << "\t\tFilter max. size: " << ml.get_filter_max_size () << std::endl;
  std::cerr << "\t\tFilter sources: " << ml.get_filter_sources () << std::endl;
...
main (int argc, char **argv)
{
  ...
  if (optind >= argc)
    {
      std::cerr << std::endl;
      std::cerr << "Error: you must enter at least one path to Searches.dat file" << std::endl;
      usage ();
      exit (EXIT_FAILURE);
    }
