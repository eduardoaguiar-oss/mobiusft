...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
show_csharedsource (const mobius::extension::app::shareaza::CSharedSource &s,
                    int indent)
{
    const std::string indent_text (indent, '\t');

    std::cout << std::endl;
    std::cout << indent_text << "Timestamp: " << s.get_timestamp ()
              << std::endl;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
show_clibraryfile (const mobius::extension::app::shareaza::CLibraryFile &f,
                   int indent)
{
    const std::string indent_text (indent, '\t');
    ...
    std::cout << indent_text
              << "Last modification time: " << f.get_last_modification_time ()
              << std::endl;
    std::cout << indent_text << "Size: " << f.get_size () << std::endl;
    std::cout << indent_text << "Virtual size: " << f.get_virtual_size ()
              << std::endl;
    std::cout << indent_text << "Virtual base: " << f.get_virtual_base ()
              << std::endl;
    std::cout << indent_text << "Is shared: " << f.is_shared () << std::endl;
    std::cout << indent_text << "Hash SHA-1: " << f.get_hash_sha1 ()
              << std::endl;
    std::cout << indent_text << "Hash Tiger: " << f.get_hash_tiger ()
              << std::endl;
    std::cout << indent_text << "Hash ED2K: " << f.get_hash_ed2k ()
              << std::endl;
    ...
    std::cout << indent_text
              << "B_Cached preview: " << f.get_b_cached_preview () << std::endl;
    std::cout << indent_text << "B_Bogus: " << f.get_b_bogus () << std::endl;
    std::cout << indent_text << "B_metadata_auto: " << f.get_b_metadata_auto ()
              << std::endl;
    std::cout << indent_text << "Metadata date/time: " << f.get_metadata_time ()
              << std::endl;
    std::cout << indent_text << "Rating: " << f.get_rating () << std::endl;
    std::cout << indent_text << "Comments: " << f.get_comments () << std::endl;
    std::cout << indent_text << "Share tags: " << f.get_share_tags ()
              << std::endl;
    std::cout << indent_text << "Hits total: " << f.get_hits_total ()
              << std::endl;
    std::cout << indent_text << "Uploads started: " << f.get_uploads_started ()
              << std::endl;
...
void
show_clibraryfolder (
    const mobius::extension::app::shareaza::CLibraryFolder &folder,
    int indent = 1)
{
    const std::string indent_text (indent, '\t');
    ...
    std::cout << indent_text << "  Is shared: " << folder.is_shared ()
              << std::endl;
    std::cout << indent_text << "  Is expanded: " << folder.is_expanded ()
              << std::endl;
...
show_clibrary (const std::string &path)
{
    ...
    std::cout << "\tLast modification time: "
              << clib.get_last_modification_time () << std::endl;
    std::cout << "\tVersion: " << clib.get_version () << std::endl;
    std::cout << "\tNext index: " << clib.get_next_index () << std::endl;
    std::cout << "\tIndex map count: " << clib.get_index_map_count ()
              << std::endl;
    std::cout << "\tName map count: " << clib.get_name_map_count ()
              << std::endl;
    std::cout << "\tPath map count: " << clib.get_path_map_count ()
              << std::endl;
...
main (int argc, char **argv)
{
    ...
    if (optind >= argc)
    {
        std::cerr << std::endl;
        std::cerr
            << "Error: you must enter at least one path to Library.dat file"
            << std::endl;
        usage ();
        exit (EXIT_FAILURE);
    }
