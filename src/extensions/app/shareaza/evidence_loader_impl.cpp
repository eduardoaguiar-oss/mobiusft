    ...
    const mobius::core::io::folder &folder)
{
    ...
    for (const auto &f :
         w.get_files_by_pattern ("appdata/local/shareaza/incomplete/*.sd"))
        _decode_sd_file (f);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_scan_canonical_shareaza_data_folder (
    const mobius::core::io::folder &folder)
{
    ...
    if (lib_f && lib2_f &&
        get_library_timestamp (lib2_f) > get_library_timestamp (lib_f))
        lib_f = lib2_f;
...
evidence_loader_impl::_decode_library_dat_file (const mobius::core::io::file &f)
{
    ...
    try
    {
        ...
        {
            log.info (__LINE__,
                      "File is not an instance of Library.dat. Path: " +
                          f.get_path ());
            return;
        }
        ...
        for (const auto &cfile : clib.get_all_files ())
        {
            ...
            lf.metadata.set ("flag_uploaded",
                             lf.flag_uploaded ? "true" : "false");
            ...
            for (const auto &source : cfile.get_sources ())
            {
                ...
                rf.metadata.set ("uploads_started",
                                 cfile.get_uploads_started ());
                rf.metadata.set ("last_modification_time",
                                 cfile.get_last_modification_time ());
                rf.metadata.set ("metadata_time", cfile.get_metadata_time ());

                if (thumbnail)
                {
                    rf.metadata.set ("thumbnail_last_write_time",
                                     thumbnail->last_write_time);
...
evidence_loader_impl::_decode_profile_xml_file (const mobius::core::io::file &f)
{
    ...
    try
    {
        if (!account_.f || (account_.f.is_deleted () && !f.is_deleted ()))
        {
            ...
            acc.gnutella_guid = mobius::core::string::toupper (
                root.get_property_by_path ("gnutella/guid"));
            acc.bittorrent_guid = mobius::core::string::toupper (
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_decode_searches_dat_file (
    const mobius::core::io::file &f)
{
    ...
    try
    {
        ...
        {
            log.info (__LINE__,
                      "File is not an instance of Searches.dat. Path: " +
                          f.get_path ());
            return;
        }

        log.info (__LINE__,
                  "Searches.dat file decoded. Path: " + f.get_path ());
        ...
        for (const auto &s : searches_dat.get_searches ())
        {
            ...
            st.metadata.set ("flag_receive",
                             cmanagedsearch.get_flag_receive ());
            st.metadata.set ("flag_allow_g1", cmanagedsearch.get_allow_g1 ());
            st.metadata.set ("flag_allow_g2", cmanagedsearch.get_allow_g2 ());
            st.metadata.set ("flag_allow_ed2k",
                             cmanagedsearch.get_allow_ed2k ());
            ...
            st.metadata.set ("cmanagedsearch_version",
                             cmanagedsearch.get_version ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_decode_shareaza_db3_file (
    const mobius::core::io::file &f)
{
    mobius::core::log log (__FILE__, __FUNCTION__);
    ...
    try
    {
        ...
        {
            log.info (__LINE__,
                      "File is not an instance of Shareaza.db3. Path: " +
                          f.get_path ());
            return;
        }

        log.info (__LINE__,
                  "Shareaza.db3 file decoded. Path: " + f.get_path ());

        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        // Update account cache, if necessary
        // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        if (!account_.shareaza_db3_f ||
            (account_.shareaza_db3_f.is_deleted () && !f.is_deleted ()))
        {
...
evidence_loader_impl::_decode_sd_file (const mobius::core::io::file &f)
{
    ...
    try
    {
        ...
        if (!sd)
        {
            log.info (__LINE__, "File is not an instance of CDownload. Path: " +
                                    f.get_path ());
            return;
        }
        ...
        for (const auto &source : sd.get_sources ())
        {
            ...
            rf.metadata.set ("total_downloaded",
                             btinfo.get_total_downloaded ());
