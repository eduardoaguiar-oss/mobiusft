#ifndef MOBIUS_EXTENSION_APP_SHAREAZA_EVIDENCE_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_SHAREAZA_EVIDENCE_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Shareaza <i>evidence_processor</i> implementation class
