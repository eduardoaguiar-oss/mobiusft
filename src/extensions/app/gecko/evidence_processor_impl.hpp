#ifndef MOBIUS_EXTENSION_APP_GECKO_EVIDENCE_PROCESSOR_IMPL_HPP
#define MOBIUS_EXTENSION_APP_GECKO_EVIDENCE_PROCESSOR_IMPL_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Gecko <i>evidence_processor</i> implementation class
