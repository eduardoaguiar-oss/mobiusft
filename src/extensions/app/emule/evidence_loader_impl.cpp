...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_scan_canonical_emule_config_folder (
    const mobius::core::io::folder &folder)
{
    account_ = {};
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_decode_preferences_dat_file (
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_decode_preferences_ini_file (
    const mobius::core::io::file &f)
{
    mobius::core::log log (__FILE__, __FUNCTION__);
    ...
    try
    {
            ...
            (account_.preferences_ini_f.is_deleted () && !f.is_deleted ()))
        {
            ...
            if (ini.has_value ("emule", "autostart"))
                account_.auto_start =
                    ini.get_value ("emule", "autostart") == "1";
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_decode_statistics_ini_file (
    const mobius::core::io::file &f)
{
    mobius::core::log log (__FILE__, __FUNCTION__);
    ...
    try
    {
            ...
            (account_.statistics_ini_f.is_deleted () && !f.is_deleted ()))
        {
            ...
            if (ini.has_value ("statistics", "TotalDownloadedBytes"))
                account_.total_downloaded_bytes = std::stol (
                    ini.get_value ("statistics", "TotalDownloadedBytes"));

            if (ini.has_value ("statistics", "TotalUploadedBytes"))
                account_.total_uploaded_bytes = std::stol (
                    ini.get_value ("statistics", "TotalUploadedBytes"));

            if (ini.has_value ("statistics", "DownCompletedFiles"))
                account_.download_completed_files = std::stol (
                    ini.get_value ("statistics", "DownCompletedFiles"));
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_decode_preferenceskad_dat_file (
    const mobius::core::io::file &f)
{
    mobius::core::log log (__FILE__, __FUNCTION__);
    ...
    try
    {
            ...
            (account_.preferenceskad_dat_f.is_deleted () && !f.is_deleted ()))
        {
            ...
            account_.kamdelia_guid = mobius::core::string::to_hex (c1, 8) +
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
evidence_loader_impl::_decode_key_index_dat_file (
    const mobius::core::io::file &f)
{
    mobius::core::log log (__FILE__, __FUNCTION__);
    ...
    try
    {
        ...
        {
            log.info (__LINE__,
                      "File is not an instance of KeyIndex.dat. Path: " +
                          f.get_path ());
            return;
        }

        log.info (__LINE__,
                  "KeyIndex.dat file decoded. Path: " + f.get_path ());
...
evidence_loader_impl::_decode_known_met_file (const mobius::core::io::file &f)
{
    ...
    try
    {
        ...
        if (!known_met)
        {
            log.info (__LINE__, "File is not an instance of Known.met. Path: " +
                                    f.get_path ());
            return;
        }
