...
// @return Name struct
// @see CKeyEntry::ReadPublishTrackingDataFromFile
// (srchybrid/kademlia/kademlia/Entry.cpp)
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static file_key_index_dat::name
_decode_name (mobius::core::decoder::data_decoder &decoder,
...
_decode_source (mobius::core::decoder::data_decoder &decoder,
                std::uint32_t version)
{
...
_decode_key (mobius::core::decoder::data_decoder &decoder,
             std::uint32_t version)
{
