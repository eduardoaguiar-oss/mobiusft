...
static const std::vector<std::string> button_text_ = {
    "",          "gtk-ok",    "gtk-cancel",
    "gtk-yes",   "gtk-no",    "gtk-open",
    "gtk-save",  "gtk-close", "gtk-discard",
    "gtk-apply", "Reset",     "Restore Defaults",
    "gtk-help",  "Save All",  "Yes to All",
    "No to All", "Abort",     "Retry",
    "Ignore"};
...
message_dialog_impl::run ()
{
    ...
    case type::error:
        ...
        break;
    default:
        gtk_type = GTK_MESSAGE_OTHER;
        break;
    }

    // create dialog
    GtkWidget *dialog =
        gtk_message_dialog_new (nullptr, GTK_DIALOG_MODAL, gtk_type,
                                GTK_BUTTONS_NONE, "%s", text_.c_str ());

    // set informative text, if necessary
    if (!informative_text_.empty ())
        gtk_message_dialog_format_secondary_text (
            GTK_MESSAGE_DIALOG (dialog), "%s", informative_text_.c_str ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void
message_dialog_impl::_add_button (const std::string &text,
                                  response_type response_id)
{
    // check if response_id is already used
    if (std::find_if (buttons_.begin (), buttons_.end (),
                      [response_id] (auto p)
                      { return p.second == response_id; }) != buttons_.end ())
        throw std::invalid_argument (
            MOBIUS_EXCEPTION_MSG ("response_id already used"));
