...
class Generator(object):
    ...
    def run(self, model):
        ...
        self.__report_title = model.report_title
        ...
        self.__generate_asap_js()
        self.__generate_evidence_js()

    # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    # @brief Create subfolder in the output folder, if necessary
    # @param args Subfolder path components
    # @return Path to the created subfolder
    # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    def __create_subfolder(self, *args):
        folder_path = os.path.join(self.__output_dir, *args)
        folder = mobius.core.io.new_folder_by_path(folder_path)
    ...
    def __generate_category_icons(self):
        ...
        categories_path = self.__create_subfolder('img', 'categories')
    ...
    def __generate_evidence_icons(self):
        ...
        evidences_path = self.__create_subfolder('img', 'evidences')
    ...
    # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    def __generate_lang_js(self):
        self.__set_status("Generating lang.js file...")

        lang_path = pymobius.mediator.call('extension.get-resource-path', EXTENSION_ID, 'lang', f'{self.__language}.js')
        f = mobius.core.io.new_file_by_path(lang_path)

        if not f.exists():
            lang_path = pymobius.mediator.call('extension.get-resource-path', EXTENSION_ID, 'lang', 'en_US.js')

        # Copy language file
        data_path = self.__create_subfolder('data')
        shutil.copy(lang_path, os.path.join(data_path, 'lang.js'))
    ...
    def __generate_model_js(self):
        ...
        if self.__report_title:
            fp.write(f'window.REPORT_TITLE = "{self.__report_title}";\n')
        ...
        for m in self.__evidence_types:
            ...
            for mv in m.get('master_views', []):
                if mv['id'] == 'table':
                    ...
                    for c in mv['columns']:
                        c_id = c['id']
                        c_name = c.get('name', None) or format_id(c_id)
                        columns.append({
                            'id': c_id,
                            'name': c_name,
    ...
    def __generate_asap_js(self):
        ...
        # Create data/asap.js file
        data_path = self.__create_subfolder('data')
        asap_js_path = os.path.join(data_path, 'asap.js')

        f = mobius.core.io.new_file_by_path(asap_js_path)
        fp = mobius.core.io.text_writer(f.new_writer())
    ...
    def __generate_item_evidence_js(self, item, evidence_type):
        ...
        base_dir = self.__create_subfolder('data', f'{item.uid:04d}')
