...
class Generator(object):
    ...
    # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    # @brief Load lang dictionary for the current language
    # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    def __load_lang_dict(self):
        ...
        if not f.exists():
            lang_path = pymobius.mediator.call('extension.get-resource-path', EXTENSION_ID, 'lang', 'en_US.txt')
            f = mobius.core.io.new_file_by_path(lang_path)
        
        if not f.exists():
            raise Exception(f"No language file found.")
        ...
        if self.__report_title:
            self.__i18n_dict['report.title'] = self.__report_title
    ...
    def __generate_lang_js(self):
        ...
        # Create data/lang.js file
        lang_js_path = os.path.join(data_path, 'lang.js')

        f = mobius.core.io.new_file_by_path(lang_js_path)
        ...
        fp.write('const LANG = ')
        json.dump(self.__i18n_dict, fp, ensure_ascii=False, separators=(',', ':'))
        fp.write(';\n\n')
        fp.write('window.LANG = LANG;\n')
    ...
    def __generate_model_js(self):
        ...
        report_title = self.__report_title or self.__i18n_dict.get("report.title")
        fp.write(f'window.VERSION = "{app.version}";\n')
        fp.write(f'window.REPORT_TITLE = "{report_title}";\n')
