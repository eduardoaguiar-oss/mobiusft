  ...
  try
    {
      ...
      if (signature == GPT_SIGNATURE)
        {
          block = ps_block.new_slice_block ("gpt.header", pos, pos + sector_size - 1);
          block.set_handled (true);
          ...
          block.set_attribute ("revision", std::to_string (revision_h) + '.' + std::to_string (revision_l));
          ...
          block.set_attribute ("partition_entry_lba", decoder.get_uint64_le ());
          block.set_attribute ("number_of_partition_entries", decoder.get_uint32_le ());
          block.set_attribute ("size_of_partition_entry", decoder.get_uint32_le ());
          block.set_attribute ("partition_entry_array_crc32", decoder.get_uint32_le ());
        }
    }
...
)
{
  ...
  if (header_block)
    {
      std::uint32_t number_of_partition_entries = header_block.get_attribute <std::int64_t> ("number_of_partition_entries");
      std::uint32_t size_of_partition_entry = header_block.get_attribute <std::int64_t> ("size_of_partition_entry");
      std::uint64_t partition_entry_lba = header_block.get_attribute <std::int64_t> ("partition_entry_lba");
      sector_type sectors = (number_of_partition_entries * size_of_partition_entry + sector_size - 1) / sector_size;
      address_type start_address = partition_entry_lba * sector_size;
      address_type end_address = (partition_entry_lba + sectors) * sector_size - 1;

      block = ps_block.new_slice_block ("gpt.table", start_address, end_address);
      block.set_handled (true);
...
)
{
  ...
  sector_type partition_entry_lba = header_block.get_attribute <std::int64_t> ("partition_entry_lba");
  std::uint32_t number_of_partition_entries = header_block.get_attribute <std::int64_t> ("number_of_partition_entries");
  ...
  for (std::uint32_t i = 0; i < number_of_partition_entries; i++)
    {
      ...
      if (type_guid == GPT_UNUSED_ENTRY)
      ...
      else
        {
          ...
          // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
          auto block = ps_block.new_slice_block ("partition", start_address, end_address);
...
)
{
  ...
  if (primary_header_block)
    {
      primary_header_block.set_attribute ("description", "Primary GPT Header");
  ...
  ps_block.set_attribute ("revision", header_block.get_attribute <std::string> ("revision"));
  ps_block.set_attribute ("disk_guid", disk_guid);
  ps_block.set_attribute ("first_usable_lba", header_block.get_attribute <std::int64_t> ("first_usable_lba"));
  ps_block.set_attribute ("last_usable_lba", header_block.get_attribute <std::int64_t> ("last_usable_lba"));
  ps_block.set_attribute ("description", "GPT partition system - Disk GUID: " + disk_guid);

  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  // Create GPT partition tables
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  auto primary_partition_table = _create_gpt_partition_table (ps_block, primary_header_block, sector_size);
  auto backup_partition_table = _create_gpt_partition_table (ps_block, backup_header_block, sector_size);

  if (primary_partition_table)
    {
      primary_partition_table.set_attribute ("description", "Primary GPT Partition Table");
    }

  if (backup_partition_table)
    {
      backup_partition_table.set_attribute ("description", "Backup GPT Partition Table");
    }

  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  // Create GPT partitions
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  std::uint32_t partitions = _decode_gpt_partition_table (ps_block, header_block, sector_size);
