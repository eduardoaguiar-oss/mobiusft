...
chs_to_string (const mobius::core::bytearray &chs)
{
    ...
    std::uint16_t c =
        (static_cast<std::uint16_t> (chs[1] & 0xc0) << 2) | chs[2];
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
_scan_partitions (mobius::core::vfs::block &ps_block,
                  sector_size_type sector_size, sector_type sector = 0)
{
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    // Create MBR entry
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    auto mbr_block = ps_block.new_slice_block ((sector == 0) ? "mbr" : "embr",
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (sectors)
        {
            address_type start_address =
                static_cast<sector_type> (starting_sector) * sector_size;
            address_type end_address =
                static_cast<sector_type> (starting_sector + sectors) *
                    sector_size -
                1;
            bool is_bootable = (drive_index & 0x80);
            bool is_primary = (sector == 0);
            bool is_extended = (type == 0x05 || type == 0x0f || type == 0x15 ||
                                type == 0x1f || type == 0x85);
            bool is_logical = (!is_primary && !is_extended);
            bool is_hidden = bool (
                type == 0x14 || type == 0x15 || type == 0x16 || type == 0x17 ||
                type == 0x1b || type == 0x1c || type == 0x1e || type == 0x1f);
            ...
            {
                auto partition_block = ps_block.new_slice_block (
                    "partition", start_address, end_address);

                partition_block.set_attribute ("start_sector", starting_sector);
                partition_block.set_attribute ("end_sector",
                                               starting_sector + sectors - 1);
                ...
                partition_block.set_attribute ("size",
                ...
                partition_block.set_attribute ("is_writeable",
                                               !is_copy_protected);
                partition_block.set_attribute (
                    "type", "0x" + mobius::core::string::to_hex (type, 2));
                partition_block.set_attribute ("type_description",
                                               type_description);
                partition_block.set_attribute ("drive_index", drive_index);
                partition_block.set_attribute ("start_chs",
                                               chs_to_string (start_chs));
                partition_block.set_attribute ("end_chs",
                                               chs_to_string (end_chs));
         ...
         std::vector<mobius::core::vfs::block> &)
{
    ...
    for (auto &b : ps_block.get_children ())
    {
        ...
        {
            std::string type_description =
                static_cast<std::string> (b.get_attribute ("type_description"));
            std::string type =
                static_cast<std::string> (b.get_attribute ("type"));
    ...
    ps_block.set_attribute ("description",
                            "DOS partition system - Disk ID: " + disk_id);
