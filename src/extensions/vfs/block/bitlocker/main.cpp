 ...
 * Implementing BitLocker Drive Encryption for Forensic Analysis - Jesse D.
 Kornblum
 ...
 * =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
 * 1. Bitlocker volume data is encrypted using the FVEK (Full Volume Encryption
 Key) key
 * 2. FVEK key is encrypted using VMK (Volume Master Key) keys
 * 3. VMK key can be stored encrypted multiple types for a single volume, using
...
static const std::string BDE_USED_DISK_SPACE_ONLY_GUID =
...
static const std::map<std::uint16_t, const std::string> PROTECTION_DESCRIPTION =
    {
        {0x0000, "Clear key"},    {0x0100, "TPM"},
        {0x0200, "Startup Key"},  {0x0500, "TPM and PIN"},
        {0x0800, "Recovery Key"}, {0x2000, "User Password"},
};
...
_decode_fve_metadata_entries (const mobius::core::bytearray &);
...
_get_protection_description (std::uint16_t flag)
{
    ...
    return "Unknown protection type (0x" +
           mobius::core::string::to_hex (flag, 4) + ')';
}
...
_get_encryption_description (std::uint16_t type)
{
    ...
    return "Unknown encryption type (0x" +
           mobius::core::string::to_hex (type, 4) + ')';
}
...
_decode_fve_metadata_0008 (const mobius::core::bytearray &data)
{
    ...
    auto unknown_1 =
        decoder.get_uint16_le (); // maybe status (active, deleted)?
    ...
    log.debug (__LINE__, "Protection flag: 0x" +
                             mobius::core::string::to_hex (protection_flag, 4));
...
_decode_fve_metadata_entry (mobius::core::decoder::data_decoder &decoder)
{
    ...
    log.debug (__LINE__,
               "Data size: " + std::to_string (size > 8 ? size - 8 : 0));
    log.debug (__LINE__,
               "Entry type: 0x" + mobius::core::string::to_hex (entry_type, 4));
    log.debug (__LINE__,
               "Value type: 0x" + mobius::core::string::to_hex (value_type, 4));
    ...
    default:
        log.development (__LINE__,
                         "New value type found: 0x" +
                             mobius::core::string::to_hex (value_type, 4));
        break;
    };
...
_decode_fve_metadata (mobius::core::vfs::block &bde_block, std::uint64_t offset)
{
    ...
    if (version == 2)
    {
        ...
        decoder.skip (4);
        bde_block.set_attribute ("volume_header_sectors",
    ...
    auto data = decoder.get_bytearray_by_size (data_size);
    auto encryption_description =
        _get_encryption_description (encryption_method);

    bde_block.set_attribute ("volume_guid", volume_guid);
    bde_block.set_attribute ("nonce_counter", nonce_counter);
    bde_block.set_attribute (
        "encryption_method",
        "0x" + mobius::core::string::to_hex (encryption_method, 8));
    ...
    log.debug (__LINE__,
               "Encryption method: " + encryption_description + " (0x" +
                   mobius::core::string::to_hex (encryption_method, 8));
    ...
    for (const auto &entry : _decode_fve_metadata_entries (data))
    {
        ...
        {
            auto v =
                bde_block.get_attribute<std::vector<mobius::core::pod::data>> (
                    "vmk");
            v.push_back (entry);
        ...
        case 0x0007:
        {
            ...
            if (v.size () > 0)
                bde_block.set_attribute ("computer_name", v[0]);
            if (v.size () > 1)
                bde_block.set_attribute ("logical_drive", v[1]);
            if (v.size () > 2)
                bde_block.set_attribute ("creation_date", v[2]);
        }
        break;

        default:
            log.development (__LINE__,
                             "New entry type found: 0x" +
                                 mobius::core::string::to_hex (entry_type, 4));
            break;
        };
    }
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
_decode_bpb_fields (mobius::core::decoder::data_decoder &decoder,
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
_decode_fat32_info (mobius::core::decoder::data_decoder &decoder,
                    mobius::core::vfs::block &bde_block)
{
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
_decode_extended_bpb_fields (mobius::core::decoder::data_decoder &decoder,
                             mobius::core::vfs::block &bde_block)
{
    bde_block.set_attribute (
        "logical_drive_number",
        "0x" + mobius::core::string::to_hex (decoder.get_uint8 (), 2));
    ...
    if (extended_signature)
    {
        bde_block.set_attribute (
            "volume_id",
            "0x" + mobius::core::string::to_hex (decoder.get_uint32_le (), 8));
        bde_block.set_attribute (
            "volume_label",
            mobius::core::string::rstrip (decoder.get_string_by_size (11)));
        bde_block.set_attribute (
            "filesystem_type",
            mobius::core::string::rstrip (decoder.get_string_by_size (8)));
    }
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
_decode_fve_metadata_offset_block (mobius::core::decoder::data_decoder &decoder,
                                   mobius::core::vfs::block &bde_block)
{
    mobius::core::log log (__FILE__, __FUNCTION__);
    ...
    {
        log.debug (__LINE__,
                   "FVE block " + std::to_string (i + 1) + " offset: 0x" +
         ...
         std::vector<mobius::core::vfs::block> &)
{
    ...
    if (bde_block.has_attribute ("volume_guid"))
        description +=
            " - GUID: " +
            static_cast<std::string> (bde_block.get_attribute ("volume_guid"));
...
start ()
{
    mobius::core::add_resource (
        "vfs.block.decoder.bitlocker", "Bitlocker VFS block decoder",
