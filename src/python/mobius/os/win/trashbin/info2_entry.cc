...
{
  return PyObject_IsInstance (value, reinterpret_cast<PyObject *> (&os_win_trashbin_info2_entry_t));
}
...
pymobius_os_win_trashbin_info2_entry_from_pyobject (PyObject *value)
{
  if (!pymobius_os_win_trashbin_info2_entry_check (value))
    throw std::invalid_argument (mobius::MOBIUS_EXCEPTION_MSG ("object must be an instance of mobius.os.win.trashbin.info2_entry"));
...
tp_getter_drive_number (os_win_trashbin_info2_entry_o *self)
{
  ...
  try
    {
      ret = mobius::py::pylong_from_std_uint32_t (self->obj->get_drive_number ());
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
PyTypeObject os_win_trashbin_info2_entry_t =
{
  ...
  "mobius.os.win.trashbin.info2_entry",    		// tp_name
