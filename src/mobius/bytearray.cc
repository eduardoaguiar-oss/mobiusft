...
bytearray::operator<<= (int n) noexcept
{
  ...
  for (size_type i = 0;i < siz - 1;i++)
    data_[i] = (data_[i + bytes] << bits) | (data_[i + bytes + 1] >> (8 - bits));
...
{
  return pos + b.size () <= size () && std::equal (b.begin (), b.end (), begin () + pos);
}
      ...
      for (int j = 0; j < 16; j++)
        {
          ...
          if (i + j < data_.size ())
            {
              dump += mobius::string::to_hex (data_[i + j], 2);
            }
