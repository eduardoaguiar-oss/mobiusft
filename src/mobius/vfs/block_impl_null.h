#ifndef MOBIUS_VFS_BLOCK_IMPL_NULL_H
#define MOBIUS_VFS_BLOCK_IMPL_NULL_H
...
public:
  ...
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  void
  add_parent (const mobius::vfs::block&) override
  {
    throw std::runtime_error (MOBIUS_EXCEPTION_MSG ("invalid block"));
  }
  ...
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  void
  add_child (const mobius::vfs::block&) override
  {
    throw std::runtime_error (MOBIUS_EXCEPTION_MSG ("Invalid block"));
  }
