      ...
      if (revision != datasource_revision_)
        {
          ...
          datasource_ = mobius::datasource::datasource (state);
        }
    }
...
item::impl::remove_datasource ()
{
  ...
  datasource_ = mobius::datasource::datasource ();
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
size_t
std::hash <mobius::model::item>::operator () (const mobius::model::item& item) const
{
