...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
tp_dealloc (os_win_registry_registry_data_o *self)
{
  delete self->obj;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_getter_size (os_win_registry_registry_data_o *self)
{
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_getter_type (os_win_registry_registry_data_o *self)
{
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_getter_data (os_win_registry_registry_data_o *self)
{
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_data_as_dword (os_win_registry_registry_data_o *self, PyObject *)
{
  // execute C++ function
  PyObject *ret = nullptr;
  ...
  try
    {
      ret = mobius::py::pylong_from_std_uint32_t (self->obj->get_data_as_dword ());
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_data_as_qword (os_win_registry_registry_data_o *self, PyObject *)
{
  // execute C++ function
  PyObject *ret = nullptr;
  ...
  try
    {
      ret = mobius::py::pylong_from_std_uint64_t (self->obj->get_data_as_qword ());
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_data_as_string (os_win_registry_registry_data_o *self, PyObject *args)
{
  // Parse input args
  ...
  try
    {
      ret = mobius::py::pystring_from_std_string (self->obj->get_data_as_string (arg_encoding));
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
{
  {
    (char *) "get_data_as_dword",
    (PyCFunction) tp_f_get_data_as_dword,
    METH_VARARGS,
    "get data as dword"
  },
  {
    (char *) "get_data_as_qword",
    (PyCFunction) tp_f_get_data_as_qword,
    METH_VARARGS,
    "get data as qword"
  },
  {
    (char *) "get_data_as_string",
    (PyCFunction) tp_f_get_data_as_string,
    METH_VARARGS,
    "get data as string"
  },
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief mobius.os.win.registry.registry_data: type structure
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject os_win_registry_registry_data_t =
{
  PyVarObject_HEAD_INIT (NULL, 0)
  "mobius.os.win.registry.registry_data",     			// tp_name
  sizeof (os_win_registry_registry_data_o),   			// tp_basicsize
  0,                                       		// tp_itemsize
