...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyObject *
pymobius_os_win_registry_registry_to_pyobject (mobius::os::win::registry::registry r)
{
  PyObject *ret = _PyObject_New (&os_win_registry_registry_t);

  if (ret)
    ((os_win_registry_registry_o *) ret)->obj = new mobius::os::win::registry::registry (r);
...
{
  os_win_registry_registry_o *self = (os_win_registry_registry_o *) type->tp_alloc (type, 0);

  if (self)
    self->obj = new mobius::os::win::registry::registry ();
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
tp_dealloc (os_win_registry_registry_o *self)
{
  delete self->obj;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_getter_files (os_win_registry_registry_o *self)
{
  ...
  try
    {
      ret = mobius::py::pylist_from_cpp_container (
               self->obj->get_files (),
               pymobius_os_win_registry_registry_file_to_pyobject
            );
    }
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_getter_keys (os_win_registry_registry_o *self)
{
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_add_file_by_path (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_role;
  std::string arg_path;
  ...
  try
    {
      ret = pymobius_os_win_registry_registry_file_to_pyobject (self->obj->add_file_by_path (arg_role, arg_path, arg_localpath));
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_add_file_by_url (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_role;
  std::string arg_path;
  ...
  try
    {
      ret = pymobius_os_win_registry_registry_file_to_pyobject (self->obj->add_file_by_url (arg_role, arg_path, arg_url));
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_remove_file (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_key_by_path (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_path;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_key_by_mask (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_mask;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_value_by_path (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_path;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_value_by_mask (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_mask;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_data_by_path (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_path;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_data_by_mask (os_win_registry_registry_o *self, PyObject *args)
{
  // parse input args
  std::string arg_mask;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_syskey (os_win_registry_registry_o *self, PyObject *)
{
  // execute C++ code
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief mobius.os.win.registry.registry: type structure
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject os_win_registry_registry_t =
{
  PyVarObject_HEAD_INIT (NULL, 0)
  "mobius.os.win.registry.registry",          			// tp_name
  sizeof (os_win_registry_registry_o),        			// tp_basicsize
  0,                                       		// tp_itemsize
