...
{
  return PyObject_IsInstance (pyobj, (PyObject *) &model_case_t);
}
...
tp_getter_root_item (model_case_o *self)
{
  ...
  try
    {
      ret = pymobius_model_item_to_pyobject (self->obj->get_root_item ());
    }
...
tp_f_get_path (model_case_o *self, PyObject *args)
{
  ...
  try
    {
      ret = mobius::py::pystring_from_std_string (self->obj->get_path (arg_rpath));
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
tp_f_create_path (model_case_o *self, PyObject *args)
{
  ...
  try
    {
      ret = mobius::py::pystring_from_std_string (self->obj->create_path (arg_rpath));
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
tp_f_get_item_by_uid (model_case_o *self, PyObject *args)
{
  ...
  try
    {
      ret = pymobius_model_item_to_pyobject (self->obj->get_item_by_uid (arg_uid));
    }
  catch (const std::exception& e)
    {
      mobius::py::set_runtime_error (e.what ());
...
static PyMethodDef tp_methods[] =
{
  ...
  {
    (char *) "get_item_by_uid",
    (PyCFunction) tp_f_get_item_by_uid,
    METH_VARARGS,
    "get item by UID"
  },
  {
...
tp_richcompare (PyObject *py_a, PyObject *py_b, int op)
{
  ...
  if (!pymobius_model_case_check (py_a) || !pymobius_model_case_check (py_b))
    ret = mobius::py::py_false ();

  else
    {
      auto a = * (reinterpret_cast <model_case_o *>(py_a)->obj);
      auto b = * (reinterpret_cast <model_case_o *>(py_b)->obj);
      bool rc = false;
...
PyTypeObject model_case_t =
{
  ...
  "mobius.model.case",                     		// tp_name
  sizeof (model_case_o),                   		// tp_basicsize
  0,                                       		// tp_itemsize
