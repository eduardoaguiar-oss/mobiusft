...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool
pymobius_io_text_reader_check (PyObject *pyobj)
{
  return PyObject_IsInstance (pyobj, (PyObject *) &io_text_reader_t);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_read (io_text_reader_o *self, PyObject *args)
{
...
{
  return (io_text_reader_o *) io_text_reader_t.tp_alloc (&io_text_reader_t, 0);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
tp_dealloc (io_text_reader_o *self)
{
  delete self->obj;
...
tp_new (PyTypeObject *type, PyObject *args, PyObject *)
{
  ...
  io_text_reader_o *ret = reinterpret_cast <io_text_reader_o *> (type->tp_alloc (type, 0));
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject io_text_reader_t =
{
  PyVarObject_HEAD_INIT (NULL, 0)
  "mobius.io.text_reader",               	   	// tp_name
  sizeof (io_text_reader_o),              	      	// tp_basicsize
  0,                                       		// tp_itemsize
