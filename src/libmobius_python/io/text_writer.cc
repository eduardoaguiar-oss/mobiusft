...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Check if object is an instance of <i>mobius.io.text_writer</i>
// @param pyobj Python object
// @return true/false
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool
pymobius_io_text_writer_check (PyObject *pyobj)
{
  return PyObject_IsInstance (pyobj, (PyObject *) &io_text_writer_t);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_write (io_text_writer_o *self, PyObject *args)
{
  // parse input args
  std::string arg_text;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_flush (io_text_writer_o *self, PyObject *)
{
  // execute C++ function
...
{
  return (io_text_writer_o *) io_text_writer_t.tp_alloc (&io_text_writer_t, 0);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
tp_dealloc (io_text_writer_o *self)
{
  delete self->obj;
...
tp_new (PyTypeObject *type, PyObject *args, PyObject *)
{
  ...
  io_text_writer_o *ret = reinterpret_cast <io_text_writer_o *> (type->tp_alloc (type, 0));
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject io_text_writer_t =
{
  PyVarObject_HEAD_INIT (NULL, 0)
  "mobius.io.text_writer",               	        // tp_name
  sizeof (io_text_writer_o),              	        // tp_basicsize
  0,                                       		// tp_itemsize
