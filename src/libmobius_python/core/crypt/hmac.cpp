...
{
    return mobius::py::isinstance (value, &core_crypt_hmac_t);
}
...
{
    return mobius::py::to_pyobject_nullable<core_crypt_hmac_o> (
        obj, &core_crypt_hmac_t);
}
...
{
    return mobius::py::from_pyobject<core_crypt_hmac_o> (value,
                                                         &core_crypt_hmac_t);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject core_crypt_hmac_t = {
    PyVarObject_HEAD_INIT (nullptr, 0)        // header
    "mobius.core.crypt.hmac",                 // tp_name
    sizeof (core_crypt_hmac_o),               // tp_basicsize
    0,                                        // tp_itemsize
