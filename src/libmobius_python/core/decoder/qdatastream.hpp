...
// Functions
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
mobius::py::pytypeobject new_core_decoder_qdatastream_type ();
bool pymobius_core_decoder_qdatastream_check (PyObject *);
...
pymobius_core_decoder_qdatastream_from_pyobject (PyObject *);
