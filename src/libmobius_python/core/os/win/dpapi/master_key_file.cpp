...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @file master_key_file.cc C++ API
// <i>mobius.core.os.win.dpapi.master_key_file</i> class wrapper
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool
pymobius_core_os_win_dpapi_master_key_file_check (PyObject *pyobj)
{
    return PyObject_IsInstance (
        pyobj, (PyObject *) &core_os_win_dpapi_master_key_file_t);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyObject *
pymobius_core_os_win_dpapi_master_key_file_to_pyobject (
    mobius::core::os::win::dpapi::master_key_file obj)
{
    PyObject *ret = _PyObject_New (&core_os_win_dpapi_master_key_file_t);

    if (ret)
        ((core_os_win_dpapi_master_key_file_o *) ret)->obj =
            new mobius::core::os::win::dpapi::master_key_file (obj);
...
pymobius_core_os_win_dpapi_master_key_file_from_pyobject (PyObject *pyobj)
{
    return *(
        reinterpret_cast<core_os_win_dpapi_master_key_file_o *> (pyobj)->obj);
}
...
tp_getter_master_key (core_os_win_dpapi_master_key_file_o *self, void *)
{
    ...
    try
    {
        ret = pymobius_core_os_win_dpapi_master_key_to_pyobject (
            self->obj->get_master_key ());
    }
    catch (const std::exception &e)
    {
        mobius::py::set_runtime_error (e.what ());
...
tp_new (PyTypeObject *type, PyObject *args, PyObject *)
{
    ...
    core_os_win_dpapi_master_key_file_o *ret =
        reinterpret_cast<core_os_win_dpapi_master_key_file_o *> (
    ...
    if (ret)
    {
        ...
        {
            ret->obj =
                new mobius::core::os::win::dpapi::master_key_file (arg_reader);
        }
        catch (const std::exception &e)
        {
            Py_DECREF (ret);
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject core_os_win_dpapi_master_key_file_t = {
    PyVarObject_HEAD_INIT (nullptr, 0)            // header
    "mobius.core.os.win.dpapi.master_key_file",   // tp_name
    sizeof (core_os_win_dpapi_master_key_file_o), // tp_basicsize
    0,                                            // tp_itemsize
