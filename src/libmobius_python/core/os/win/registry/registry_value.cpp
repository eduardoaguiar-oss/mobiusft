...
pymobius_core_os_win_registry_registry_value_to_pyobject (
    mobius::core::os::win::registry::registry_value r)
{
    PyObject *ret = nullptr;

    if (r)
    {
        ret = _PyObject_New (&core_os_win_registry_registry_value_t);

        if (ret)
            ((core_os_win_registry_registry_value_o *) ret)->obj =
                new mobius::core::os::win::registry::registry_value (r);
    }
    else
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
tp_dealloc (core_os_win_registry_registry_value_o *self)
{
    delete self->obj;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief mobius.core.os.win.registry.registry_value: type structure
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject core_os_win_registry_registry_value_t = {
    PyVarObject_HEAD_INIT (nullptr, 0)              // header
    "mobius.core.os.win.registry.registry_value",   // tp_name
    sizeof (core_os_win_registry_registry_value_o), // tp_basicsize
    0,                                              // tp_itemsize
