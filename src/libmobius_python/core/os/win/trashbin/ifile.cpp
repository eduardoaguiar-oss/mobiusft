...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @file ifile.cc C++ API <i>mobius.core.os.win.trashbin.ifile</i> class wrapper
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool
pymobius_core_os_win_trashbin_ifile_check (PyObject *value)
{
    return PyObject_IsInstance (
        value, reinterpret_cast<PyObject *> (&core_os_win_trashbin_ifile_t));
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyObject *
pymobius_core_os_win_trashbin_ifile_to_pyobject (
    const mobius::core::os::win::trashbin::ifile &obj)
{
    PyObject *ret = _PyObject_New (&core_os_win_trashbin_ifile_t);

    if (ret)
        ((core_os_win_trashbin_ifile_o *) ret)->obj =
            new mobius::core::os::win::trashbin::ifile (obj);
...
pymobius_core_os_win_trashbin_ifile_from_pyobject (PyObject *value)
{
    if (!pymobius_core_os_win_trashbin_ifile_check (value))
        throw std::invalid_argument (MOBIUS_EXCEPTION_MSG (
            "object must be an instance of mobius.core.os.win.trashbin.ifile"));

    return *(reinterpret_cast<core_os_win_trashbin_ifile_o *> (value)->obj);
}
...
tp_new (PyTypeObject *type, PyObject *args, PyObject *)
{
    ...
    core_os_win_trashbin_ifile_o *ret =
        reinterpret_cast<core_os_win_trashbin_ifile_o *> (
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
tp_dealloc (core_os_win_trashbin_ifile_o *self)
{
    delete self->obj;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject core_os_win_trashbin_ifile_t = {
    PyVarObject_HEAD_INIT (nullptr, 0)        // header
    "mobius.core.os.win.trashbin.ifile",      // tp_name
    sizeof (core_os_win_trashbin_ifile_o),    // tp_basicsize
    0,                                        // tp_itemsize
