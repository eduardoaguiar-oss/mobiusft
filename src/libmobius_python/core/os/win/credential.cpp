...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @file credential.cc C++ API <i>mobius.core.os.win.credential</i> class
// wrapper
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool
pymobius_core_os_win_credential_check (PyObject *pyobj)
{
    return PyObject_IsInstance (pyobj, (PyObject *) &core_os_win_credential_t);
}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyObject *
pymobius_core_os_win_credential_to_pyobject (
    mobius::core::os::win::credential obj)
{
    PyObject *ret = _PyObject_New (&core_os_win_credential_t);

    if (ret)
        ((core_os_win_credential_o *) ret)->obj =
            new mobius::core::os::win::credential (obj);
...
pymobius_core_os_win_credential_from_pyobject (PyObject *pyobj)
{
    return *(reinterpret_cast<core_os_win_credential_o *> (pyobj)->obj);
}
...
tp_new (PyTypeObject *type, PyObject *args, PyObject *)
{
    ...
    core_os_win_credential_o *ret =
        reinterpret_cast<core_os_win_credential_o *> (type->tp_alloc (type, 0));
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static void
tp_dealloc (core_os_win_credential_o *self)
{
    delete self->obj;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyTypeObject core_os_win_credential_t = {
    PyVarObject_HEAD_INIT (nullptr, 0)        // header
    "mobius.core.os.win.credential",          // tp_name
    sizeof (core_os_win_credential_o),        // tp_basicsize
    0,                                        // tp_itemsize
