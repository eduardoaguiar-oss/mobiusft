...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief function mobius.core.system.get_devices
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyObject *
func_get_devices (PyObject *, PyObject *args)
{
    ...
    try
    {
        ...
        std::copy_if (dev_list.begin (), dev_list.end (),
                      ...
                      {
                          return (arg_subsystem.empty () ||
                                  arg_subsystem == dev.get_subsystem ()) &&
                                 (arg_type.empty () ||
                                  arg_type == dev.get_type ());
                      });

        ret = mobius::py::pylist_from_cpp_container (
            selected_dev_list, pymobius_core_system_device_to_pyobject);
    }
    catch (const std::exception &e)
    {
