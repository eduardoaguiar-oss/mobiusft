#ifndef LIBMOBIUS_PYTHON_PYFUNCTION_HPP
#define LIBMOBIUS_PYTHON_PYFUNCTION_HPP
    ...
    operator() (Args... args)
    {
        if (!impl_)
            throw std::runtime_error (
                MOBIUS_EXCEPTION_MSG ("null function cannot be called")
            );
        ...
        if (!t)
            throw std::runtime_error (
                MOBIUS_EXCEPTION_MSG ("could not build arguments")
            );
