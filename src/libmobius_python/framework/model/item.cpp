...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @file item.cc C++ API <i>mobius.framework.model.item</i> class wrapper
...
tp_getter_case (framework_model_item_o *self, void *)
{
    ...
    {
        ret =
            pymobius_framework_model_case_to_pyobject (self->obj->get_case ());
    }
    catch (const std::exception &e)
    {
        mobius::py::set_runtime_error (e.what ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_child_count (framework_model_item_o *self, PyObject *)
{
    // execute C++ code
    PyObject *ret = nullptr;
    ...
    {
        ret =
            mobius::py::pylong_from_std_int64_t (self->obj->get_child_count ());
    }
    catch (const std::exception &e)
    {
        mobius::py::set_runtime_error (e.what ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_children (framework_model_item_o *self, PyObject *)
{
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_parent (framework_model_item_o *self, PyObject *)
{
    // execute C++ code
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_new_child (framework_model_item_o *self, PyObject *args)
{
    // parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_remove (framework_model_item_o *self, PyObject *)
{
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_move (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    int arg_idx;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_expand_masks (framework_model_item_o *self, PyObject *)
{
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_data_path (framework_model_item_o *self, PyObject *args)
{
    // parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_create_data_path (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_rpath;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_has_attribute (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_id;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_attribute (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_id;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_attribute (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_id;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_remove_attribute (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_id;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_attributes (framework_model_item_o *self, PyObject *)
{
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_has_datasource (framework_model_item_o *self, PyObject *)
{
    // Execute C++ function
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_datasource (framework_model_item_o *self, PyObject *)
{
    // Execute C++ function
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_datasource (framework_model_item_o *self, PyObject *args)
{
    // Parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_remove_datasource (framework_model_item_o *self, PyObject *)
{
    // execute C++ code
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_has_ant (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_id;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_ant (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_id;
    std::string arg_name;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_reset_ant (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_id;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_remove_ants (framework_model_item_o *self, PyObject *)
{
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_ants (framework_model_item_o *self, PyObject *)
{
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_new_evidence (framework_model_item_o *self, PyObject *args)
{
    // Parse input args
    std::string arg_type;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_add_evidence (framework_model_item_o *self, PyObject *args)
{
    // Parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_evidences (framework_model_item_o *self, PyObject *args)
{
    // Parse input args
    std::string arg_type;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_remove_evidences (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_type;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_count_evidences (framework_model_item_o *self, PyObject *args)
{
    // parse input args
    std::string arg_type;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_add_event (framework_model_item_o *self, PyObject *args)
{
    // Parse input args
    std::string arg_text;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_events (framework_model_item_o *self, PyObject *)
{
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_new_connection (framework_model_item_o *self, PyObject *)
{
    // execute C++ function
    PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_new_transaction (framework_model_item_o *self, PyObject *)
{
    // execute C++ function
    PyObject *ret = nullptr;
...
static PyMethodDef tp_methods[] = {
    {"get_child_count", (PyCFunction) tp_f_get_child_count, METH_VARARGS,
    ...
    {"expand_masks", (PyCFunction) tp_f_expand_masks, METH_VARARGS,
     "Expand value masks using item's attributes"},
    {"get_data_path", (PyCFunction) tp_f_get_data_path, METH_VARARGS,
     "Get item data path"},
    {"create_data_path", (PyCFunction) tp_f_create_data_path, METH_VARARGS,
    ...
    {"get_attributes", (PyCFunction) tp_f_get_attributes, METH_VARARGS,
     "Get attributes"},
    {"has_datasource", (PyCFunction) tp_f_has_datasource, METH_VARARGS,
     "Check if item has datasource"},
    {"get_datasource", (PyCFunction) tp_f_get_datasource, METH_VARARGS,
     "Get datasource"},
    {"set_datasource", (PyCFunction) tp_f_set_datasource, METH_VARARGS,
     "Set datasource"},
    {"remove_datasource", (PyCFunction) tp_f_remove_datasource, METH_VARARGS,
     "Remove datasource"},
    {"has_ant", (PyCFunction) tp_f_has_ant, METH_VARARGS,
    ...
    {"remove_ants", (PyCFunction) tp_f_remove_ants, METH_VARARGS,
     "Remove ANTs"},
    {"get_ants", (PyCFunction) tp_f_get_ants, METH_VARARGS, "Get ANTs"},
    {"new_evidence", (PyCFunction) tp_f_new_evidence, METH_VARARGS,
     "Create new evidence"},
    {"add_evidence", (PyCFunction) tp_f_add_evidence, METH_VARARGS,
     "Add evidence"},
    {"get_evidences", (PyCFunction) tp_f_get_evidences, METH_VARARGS,
    ...
    {"new_transaction", (PyCFunction) tp_f_new_transaction, METH_VARARGS,
     "Create new transaction for case database"},
...
{
    framework_model_item_o *self =
        (framework_model_item_o *) type->tp_alloc (type, 0);
...
tp_getattro (PyObject *o, PyObject *name)
{
    ...
    try
    {
        ...
        if (ret == nullptr)
        {
            ...
            auto self = reinterpret_cast<framework_model_item_o *> (o);
...
tp_richcompare (PyObject *py_a, PyObject *py_b, int op)
{
    ...
    if (!pymobius_framework_model_item_check (py_a) ||
        !pymobius_framework_model_item_check (py_b))
        ret = mobius::py::py_false ();

    else
    {
        auto a = *(reinterpret_cast<framework_model_item_o *> (py_a)->obj);
        auto b = *(reinterpret_cast<framework_model_item_o *> (py_b)->obj);
        bool rc = false;
