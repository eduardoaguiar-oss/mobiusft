...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//! \file window.cc C++ API <i>mobius.ui.window</i> class wrapper
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_sensitive (ui_window_o *self, PyObject *args)
{
  // Parse input args
  bool arg_flag;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_visible (ui_window_o *self, PyObject *args)
{
  // Parse input args
  bool arg_flag;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_show_all (ui_window_o *self, PyObject *)
{
  // Execute C++ function
  try
    {
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_destroy (ui_window_o *self, PyObject *)
{
  // Execute C++ function
  try
    {
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_title (ui_window_o *self, PyObject *args)
{
  // Parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_title (ui_window_o *self, PyObject *)
{
  // Execute C++ function
...
tp_f_set_icon (ui_window_o *self, PyObject *args)
{
  ...
  try
    {
      arg_icon = mobius::py::get_arg_as_cpp (args, 0, pymobius_ui_icon_from_pyobject);
    }
  catch (const std::exception& e)
    {
      mobius::py::set_invalid_type_error (e.what ());
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_border_width (ui_window_o *self, PyObject *args)
{
  // Parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_content (ui_window_o *self, PyObject *)
{
  // Execute C++ function
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_remove_content (ui_window_o *self, PyObject *)
{
  // Execute C++ function
  try
    {
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_position (ui_window_o *self, PyObject *args)
{
  // Parse input args
  std::uint32_t arg_x;
  std::uint32_t arg_y;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_position (ui_window_o *self, PyObject *)
{
  // Execute C++ function
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_size (ui_window_o *self, PyObject *args)
{
  // Parse input args
  std::uint32_t arg_width;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_get_size (ui_window_o *self, PyObject *)
{
  // Execute C++ function
  PyObject *ret = nullptr;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_set_callback (ui_window_o *self, PyObject *args)
{
  // Parse input args
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyObject *
tp_f_reset_callback (ui_window_o *self, PyObject *args)
{
  // Parse input args
  std::string arg_event_id;
...
static PyMethodDef tp_methods[] =
{
  ...
  {"show_all", (PyCFunction) tp_f_show_all, METH_VARARGS, "Show widget and its children"},
  ...
  {"set_content", (PyCFunction) tp_f_set_content, METH_VARARGS, "Set content"},
  {"get_content", (PyCFunction) tp_f_get_content, METH_VARARGS, "Get content"},
  {"remove_content", (PyCFunction) tp_f_remove_content, METH_VARARGS, "Remove content"},
  {"set_position", (PyCFunction) tp_f_set_position, METH_VARARGS, "Set position"},
  {"get_position", (PyCFunction) tp_f_get_position, METH_VARARGS, "Get position"},
...
{
  ui_window_o *ret = reinterpret_cast <ui_window_o *> (type->tp_alloc (type, 0));
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
static PyTypeObject ui_window_t =
{
  PyVarObject_HEAD_INIT (NULL, 0)
  "mobius.ui.window",                      		// tp_name
  sizeof (ui_window_o),                    		// tp_basicsize
  0,                                       		// tp_itemsize
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Create <i>mobius.ui.window</i> type
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
mobius::py::pytypeobject
new_ui_window_type ()
{
  mobius::py::pytypeobject type (&ui_window_t);
  type.create ();
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool
pymobius_ui_window_check (PyObject *value)
{
  return mobius::py::isinstance (value, &ui_window_t);
}
