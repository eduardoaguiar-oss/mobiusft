...
namespace
{
static constexpr std::uint64_t MAX_PYTHON_TIMESTAMP = 253402300799; // 9999-12-31 23:59:59
} // namespace

// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Function new_datetime_from_unix_timestamp
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
PyObject*
func_new_datetime_from_unix_timestamp (PyObject *, PyObject *args)
{
  // parse input args
