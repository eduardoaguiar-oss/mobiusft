#ifndef MOBIUS_CORE_VFS_DISK_HPP
#define MOBIUS_CORE_VFS_DISK_HPP
...
public:
  ...
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  disk ();
  explicit disk (const mobius::core::pod::map&);
  ...
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  void
  set_attribute (const std::string& id, const mobius::core::pod::data& value)
  {
