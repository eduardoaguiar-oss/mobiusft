#ifndef MOBIUS_CORE_VFS_TSK_READER_IMPL_STREAM_HPP
#define MOBIUS_CORE_VFS_TSK_READER_IMPL_STREAM_HPP
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
class reader_impl_stream : public mobius::core::io::reader_impl_base
{
public:
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  // Constructors
