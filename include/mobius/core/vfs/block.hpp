#ifndef MOBIUS_CORE_VFS_BLOCK_HPP
#define MOBIUS_CORE_VFS_BLOCK_HPP
...
class block
{
  public:
    ...
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    block ();
    explicit block (const mobius::core::pod::map &);
    ...
    void
    set_attribute (
        const std::string &name, const mobius::core::pod::data &value
    )
    {
    ...
    // @return <b>true</b> if block is handled, <b>false</b> otherwise
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    bool
    is_handled () const
    {
    ...
    // @return <b>true</b> if block is complete, <b>false</b> otherwise
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    bool
    is_complete () const
    {
    ...
    // @return <b>true</b> if block is available, <b>false</b> otherwise
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    bool
    is_available () const
    {
    ...
    block new_slice_block (
        const std::string &, block::offset_type = 0, block::offset_type = -1
    );
    void add_freespaces ();
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
using block_decoder_resource_type = std::function<
    bool (const block &, std::vector<block> &, std::vector<block> &)>;

using block_builder_resource_type =
    std::function<block (const mobius::core::pod::map &)>;

// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Helper functions
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void register_block_decoder (
    const std::string &, const block_decoder_resource_type &
);
void unregister_block_decoder (const std::string &);
std::vector<block_decoder_resource_type> get_block_decoders ();
