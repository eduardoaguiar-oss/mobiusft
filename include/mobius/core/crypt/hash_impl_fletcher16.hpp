#ifndef MOBIUS_CRYPT_HASH_IMPL_FLETCHER16_HPP
#define MOBIUS_CRYPT_HASH_IMPL_FLETCHER16_HPP
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Fletcher-16 hash function implementation
// @author Eduardo Aguiar
// @see RFC 1145
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
class hash_impl_fletcher16 : public hash_impl_base
{
  public:
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
