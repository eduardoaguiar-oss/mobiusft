#ifndef MOBIUS_CRYPT_HASH_IMPL_FLETCHER32_HPP
#define MOBIUS_CRYPT_HASH_IMPL_FLETCHER32_HPP
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Fletcher-32 hash function implementation
// @author Eduardo Aguiar
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
class hash_impl_fletcher32 : public hash_impl_base
{
  public:
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  private:
    std::uint64_t sum1_ = 0xffff;
    std::uint64_t sum2_ = 0xffff;
};
