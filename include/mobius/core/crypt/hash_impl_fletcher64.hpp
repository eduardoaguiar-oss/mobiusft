#ifndef MOBIUS_CRYPT_HASH_IMPL_FLETCHER64_HPP
#define MOBIUS_CRYPT_HASH_IMPL_FLETCHER64_HPP
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief Fletcher-64 hash function implementation
// @author Eduardo Aguiar
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
class hash_impl_fletcher64 : public hash_impl_base
{
  public:
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  private:
    std::uint64_t sum1_ = 0xffffffff;
    std::uint64_t sum2_ = 0xffffffff;
};
