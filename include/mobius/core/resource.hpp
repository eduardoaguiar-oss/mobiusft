...
public:
  ...
  {
    if (!impl_)
      throw std::invalid_argument (MOBIUS_EXCEPTION_MSG ("invalid resource"));
  ...
  get_value () const
  {
    ...
    if (!p)
      throw std::invalid_argument (MOBIUS_EXCEPTION_MSG ("invalid resource type"));
...
)
{
  ...
  if (pos == std::string::npos || pos == id.size () - 1)
    throw std::invalid_argument (MOBIUS_EXCEPTION_MSG ("invalid resource ID"));
...
get_resource_value (const std::string& id)
{
  ...
  if (!r)
    throw std::runtime_error (MOBIUS_EXCEPTION_MSG ("resource '" + id + "' not found"));
