#ifndef MOBIUS_CORE_IO_READER_EVALUATOR_HPP
#define MOBIUS_CORE_IO_READER_EVALUATOR_HPP
...
public:
  ...
  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  reader_impl_evaluator (mobius::core::io::reader reader, T& functor)
    : reader_ (reader),
      functor_ (functor)
  {
  }
  ...
  {
    const mobius::core::bytearray data = reader_.read (size);
    functor_ (data);
    return data;
  }

private:
  mobius::core::io::reader reader_;            // nested reader
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
template <typename T> const mobius::core::io::reader
reader_evaluator (mobius::core::io::reader reader, T& functor)
{
  return mobius::core::io::reader (std::make_shared <reader_impl_evaluator<T>> (reader, functor));
}
