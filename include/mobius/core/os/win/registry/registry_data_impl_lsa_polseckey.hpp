#ifndef MOBIUS_CORE_REGISTRY_REGISTRY_DATA_IMPL_POLSECKEY_HPP
#define MOBIUS_CORE_REGISTRY_REGISTRY_DATA_IMPL_POLSECKEY_HPP
...
{
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// @brief registry_data (subtype LSA PolSecKey) implementation class
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
class registry_data_impl_lsa_polseckey : public registry_data_impl_base
{
public:
  registry_data_impl_lsa_polseckey (const mobius::core::bytearray&, const mobius::core::bytearray&);
