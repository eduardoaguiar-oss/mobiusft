#ifndef MOBIUS_FRAMEWORK_MODEL_ITEM_HPP
#define MOBIUS_FRAMEWORK_MODEL_ITEM_HPP
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
template <>
struct hash <mobius::framework::model::item>
{
  size_t operator()(const mobius::framework::model::item&) const;
};
